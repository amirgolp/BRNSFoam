...
c     The following command seems to give problems on geohpc.geu.uu.nl!
c     The next three lines can be outcommented there. However, the rate
c     output file will not be deleted then. Beware.
c      open(unit=11,file='ratesAtFinish.dat',access='append',
c     + dispose='DELETE')
c      close(11)
      endif
      open(unit=11,file='ratesAtFinish.dat',access='append')
      write(11,2000) x_pos, y_pos, z_pos, (r(l,1), l = 1, nreac)
2000  format(3e20.10, 512e20.10)
      close(11)
      endif
