...
c           
          ...
          if (ic.eq.3) then
              open(unit=101,file='C_substrate.inp',status='old')
              open(unit=102,file='C_N2O.inp',status='old')
              open(unit=103,file='C_NO.inp',status='old')
              open(unit=104,file='C_NO2.inp',status='old')
c             
            do 1003, j=1,nx,2
